...
@Composable
fun InkCanvasOverlay(state: EditorState, pageIndex: Int, onEvent: (RibbonEvent) -> Unit) {
    var currentPath by remember { mutableStateOf<Path?>(null) }
    
    val canvasModifier = if (state.isDrawingMode) {
        Modifier
            .fillMaxSize()
            .pointerInput(state.inkColor, state.inkThickness, state.isHighlighterMode, state.isEraserMode) {
                detectDragGestures(
                    onDragStart = { offset ->
    ...
    Canvas(modifier = canvasModifier) {
        // Draw existing paths for this page
        state.drawingPaths.filter { it.pageIndex == pageIndex }.forEach { dp ->
            drawPath(
                path = dp.path,
                color = dp.color,
                style = Stroke(
